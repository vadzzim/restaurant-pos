...
`modules/realtime/consumer.ts`. So the event travels outbox → worker → Kafka → the realtime consumer
group → *one* replica → the Redis adapter → the rest, and which replica consumes is not ours to
choose. The test therefore watches **both** sockets: `handleRealtimeEvent` writes `processed_events`
before it emits, so exactly one replica in the fleet ever emits a given event, and two instances
delivering the same `eventId` is the Redis fan-out and nothing else.

**One real defect fell out of the images.** `loadEnv()` in `apps/web/vite.config.ts` is not
read-only — a `NODE_ENV` in the files it reads is promoted to `process.env.VITE_USER_NODE_ENV`,
which is what Vite consults for the build's mode — and the repository-root `.env` says
`development`. Every `pnpm build` since M1 emitted a *development* bundle: 446 kB with dev warnings
and devtools hooks, against the image's 307 kB. The image had no `.env` and was the only correct
production build in the repository. Fixed; host and image now emit the same file.

**Demoable end to end:** the order lifecycle, a broker outage, reloading the tab mid-order, §19.2,
§19.3, §19.9, §19.10, `/debug` live, every §18 control, and the flag rollout with both transports.
...
- `packages/contracts` — statuses, mutations, events, the §5 shapes, `ConflictReason`, socket names,
  `TERMINALS`, the debug, simulator and flag shapes.
...
- `apps/api` — the nine-branch mutation endpoint, the two §17 kitchen adapters, the four reads,
  ...
  `modules/config/`. Tests: ten files under `pnpm -F @pos/api test`, plus
  `test/multi-instance.integration.test.ts` behind `vitest.integration.config.ts`, **excluded** from
  the default config so the normal suite still runs against a database alone.
...
- `apps/web` — the POS and kitchen screens; Pinia stores for menu, order, kitchen, connection,
  ...
  `socket.ts`, `polling.ts` and `presence-beat.ts`; `views/DebugView.vue`. `/demo` is still the M1
  placeholder (M16).
- **Images and Compose** — `apps/{api,worker,web}/Dockerfile`, `apps/web/nginx.conf`, root
  `.dockerignore`, `docker-compose.multi.yml`. The base `docker-compose.yml` `app` profile is
  unchanged and is the *development* stack: it mounts the tree and runs `tsx`.
- **Scripts and CI** — `scripts/lib/compose-run.mjs` (the shared Compose lifecycle),
  `verify-integration.mjs`, `verify-multi-instance.mjs`, `.github/workflows/ci.yml` with a second
  job that builds the three images.
...
of them new from M14. **Do not read it to start a session.** The sweep is its own pass and M15 is
not it either.
...
6. **Do not sweep the review backlog.** Eleven entries: four new M14 ones, M13's P2, two M12 P3,
   two M11 P2, two M11 P3.
...
torn down. `pnpm verify:multi --keep` is how to get the built app on :8081 in front of both
replicas if M15 wants to see itself through nginx — but `pnpm dev` on :5173 is the normal loop.
```
